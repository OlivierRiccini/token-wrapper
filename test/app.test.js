...
contract('TokenWrapper', ([appManager, user]) => {
  const INIT_VALUE = 42
  ...
  before('deploy base app', async () => {
    ...
    appBase = await TokenWrapper.new(INIT_VALUE)
  })
  ...
  beforeEach('deploy dao and app', async () => {
    ...
    const proxyAddress = await newApp(dao, 'token-wrapper', appBase.address, appManager)
    app = await TokenWrapper.at(proxyAddress)
